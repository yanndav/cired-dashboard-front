...
const auteurs = (auteurs) =>{
    return <p className="auteurs">par  
        {' ' + auteurs.map(d=> d.PRENOM + ' ' + d.NOM)}
    </p>
}

const Modules = ({modules, setShowModules}) => {
    ...
    return (
        <div className="module-select-container">
            <div className="selection">
                ...
                style={{color:d===theme&&"white",backgroundColor:d===theme&&"#0AAACB"}} 
                onClick={()=>setTheme(d)}>
                    {d}
                </span>)}
                ...
                {theme!==""&& <div className="modules">
                    
                        <span className="titre">MODULES</span>
                            {modulesTheme.map(d=><span 
                                style={{color:(module!==null && d.TITRE===module.TITRE)&&"white",backgroundColor:(module!==null && d.TITRE===module.TITRE)&&"#0AAACB"}}
                                className="module-bouton" onClick={()=> setModule(d)}>{d.TITRE}</span>)}
                ...
                <div className="module-description">
                    ...
                    <p class="select-module">Ajouter le module</p>
                </div>
                }
            </div>
            <MdKeyboardArrowUp 
