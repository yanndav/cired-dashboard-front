...
const id_gen = (CODGEO)=> CODGEO.split('').map(d=>num_to_let(d)).join('')
...
const updateVoisinnage = async (API_URL,map,geographies,voisinnage,setVoisinnage) =>{
  ...
  if(voisinnage_start.length!==0){
    ...
    suppr = voisinnage_start.filter(a=>!id_end.includes(a.properties.CODGEO[0]))
...
// Fonctions outils ---------------
  ...
  const getVoisinnage = async (API_URL,geographies,map,voisinnage,setVoisinnage,change,setTerritories,setGeographies,setRemove) =>{
    ...
    const {suppr,ajout,all} = await updateVoisinnage(API_URL,map,geographies,voisinnage,setVoisinnage)
    ...
    if(change==="drag"){
      ...
      .on("mouseover", (d)=> {
        d3.select(d.target.parentElement).select('path').attr("class",`z${ZOOM} mouseOver leaflet-interactive`)
        d3.select(d.target.parentElement).select('text').attr("class",`legendVoisin-hover`).attr("visibility","visible")
      })
    .on("mouseout", (d)=>{
    d3.select(d.target.parentElement).select('path').attr("class",`z${ZOOM} normal leaflet-interactive`)
    d3.select(d.target.parentElement).select('text').attr("class",`legendVoisin`).attr("visibility",ZOOM>10?"visible":"hidden")
  }).on("click",(d)=>{
...
const updateShape = (geographies,map,setTerritories,API_URL,setGeographies)=>{
  ...
  .attr('pointer-events','all')
  .on("mouseover", (d)=> {
    d3.select(d.target.parentElement).select('path').attr("class",`z${ZOOM} legendTerritoire-hover leaflet-interactive`)
    d3.select(d.target.parentElement).select('text').attr("class",`legendTerritoire-text-hover`).attr("visibility","visible")
  })
  .on("mouseout", (d)=>{
  d3.select(d.target.parentElement).select('path').attr("class",`z${ZOOM} legendTerritoire leaflet-interactive`)
  d3.select(d.target.parentElement).select('text').attr("class",`legendTerritoire-text`).attr("visibility",ZOOM>9?"visible":"hidden")
  }).on("click",(d)=>{
  const t = d.target.parentElement.__data__.properties
...
const removerTerritoryFromAnalysis = (e,t,setTerritories,setGeographies) =>{
    ...
    setTerritories((territories)=>territories.filter(ter=>ter._id.$oid!==id))
    setGeographies((geographies)=>geographies.filter(t=>t.properties._id.$oid !==id))
