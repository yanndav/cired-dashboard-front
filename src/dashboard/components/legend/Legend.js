...
const variables = [
  {
    ...
    definition:
      "Transformation d'un sol à caractère agricole, naturel ou forestier par des actions d'aménagement, pouvant entraîner son imperméabilisation totale ou partielle. Ce changement d'usage des sols, le plus souvent irréversible, a des conséquences qui peuvent être préjudiciables à l'environnement et à la production agricole",
  ...
  const legendEvolution = (variables) => {
    return variables.map((v, i) => {
      return (
        <>
          {" "}
          {legendTime(v.year)},{" " + v.pre_label + " "}
          <span
            className="info-variable"
  ...
  return (
    <div className="legend-container">
      <h3 className="legend-title">
        Vous étudiez {legendLocation(territories)}
      </h3>
